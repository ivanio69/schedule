...
const noteKey = (date: string, start: string, title: string) =>
  `${date}|${start}|${title}`;
const pick = (items: string[], seed: string) => {
  let hash = 0;
  for (const char of seed) hash = (hash * 31 + char.charCodeAt(0)) >>> 0;
  return items[hash % items.length];
};
type Event = {
  kind: "group" | "individual" | "rehearsal";
  start: string;
  end: string;
  title: string;
  subtitle: string;
...
export default function DashboardV2() {
  ...
  const [person, setPerson] = useState<Person | null>(null),
    ...
    [noteEvent, setNoteEvent] = useState<Event | null>(null),
  ...
  const loadProfile = async (id: string) => {
    const [sr, ir] = await Promise.all([
      fetch(`/api/profile/settings?personId=${encodeURIComponent(id)}`, {
        cache: "no-store",
      }),
      fetch(`/api/individuals?personId=${encodeURIComponent(id)}`, {
        cache: "no-store",
      }),
    ]);
    if (sr.ok) {
  ...
  const loadRehearsals = async (id: string, date: string) => {
    const r = await fetch(
      `/api/rehearsals?date=${encodeURIComponent(date)}&personId=${encodeURIComponent(id)}`,
      { cache: "no-store" },
    );
    if (r.ok) setRehearsals((await r.json()).rehearsals ?? []);
  };
  useEffect(() => {
  ...
  const groups = useMemo(
    () =>
      schedule && today >= 0
        ? getLessonsForWeek(schedule, today, week, preferences, chinaMode)
        : [],
    [schedule, today, week, preferences, chinaMode],
  );
      ...
      [
        ...
        ...individuals
          ...
          .map((x) => ({
            ...
            title: x.subject,
            subtitle: `Индивидуальное · ${x.professor}`,
            detail: x.auditorium,
            note: x.note,
            key: x.id,
          })),
        ...rehearsals
          ...
          .map((x) => ({
            ...
            title: x.subject,
            subtitle: x.blocks?.length
              ? `Репетиция · ${x.blocks.length} блоков`
              : "Репетиция",
            detail: x.responsible,
            note: x.notes,
            key: x.id,
          })),
  ...
  const currentMin = now.getHours() * 60 + now.getMinutes(),
    ...
    current = activeEvents.find(
      (e) => mins(e.start) <= currentMin && mins(e.end) > currentMin,
    ),
    next = activeEvents.find((e) => mins(e.start) > currentMin),
    past = (e: Event) => mins(e.end) <= currentMin;
  const hour = now.getHours(),
    daySeed = `${todayKey}|${person?.id ?? "guest"}`,
    nightText =
      hour >= 1 && hour < 4
        ? pick(["спи.", "сладких снов на потоках", "ты че бухаешь?"], daySeed)
        : hour >= 4 && hour < 8
          ? pick(
              ["доброе утро?", "зачем так рано", "приз за ответственность"],
              daySeed,
            )
          : null;
  const previous = next
      ? activeEvents.filter((e) => mins(e.end) <= mins(next.start)).at(-1)
      : null,
    gapMinutes = next && previous ? mins(next.start) - mins(previous.end) : 0,
    gapText =
      next && gapMinutes > 120
        ? pick(
            [
  ...
  const tomorrowKey = dateKey(tomorrowDate),
    tomorrowLessons = schedule
      ? getOccurrences(schedule, tomorrowKey).filter((x) => {
          if (x.occurrence?.status === "cancelled") return false;
          if (!lessonMatchesChinaMode(x, chinaMode)) return false;
          if (chinaMode) return true;
          const preference = preferences[x.class] ?? "both";
          return (
            preference === "both" ||
            !x.group.length ||
            x.group.includes(Number(preference))
          );
        })
      : [],
    tomorrowRehearsals = rehearsals.filter((x) => x.date === tomorrowKey),
    tomorrowIndividuals = individuals.filter((x) => x.date === tomorrowKey),
    tomorrowEvents = [
      ...tomorrowLessons.map((x) => ({
        start: x.timeStart,
        end: x.timeEnd,
        title: x.class,
      })),
      ...tomorrowRehearsals.map((x) => ({
        start: x.timeStart,
        end: x.timeEnd,
        title: x.subject,
      })),
      ...tomorrowIndividuals.map((x) => ({
        start: x.timeStart,
        end: x.timeEnd,
        title: x.subject,
      })),
    ].sort((a, b) => a.start.localeCompare(b.start)),
    todayFinished = events.length > 0 && !current && !next,
    showTomorrow = todayFinished || (!events.length && hour >= 18);
  const mySeminars = seminars.flatMap((list) =>
    list.topics
      .filter((topic) => topic.studentIds.includes(person?.id ?? ""))
      .map((topic) => ({
        listId: list.id,
        ...
        participants: topic.studentNames ?? [],
      })),
  );
  const countdown = (v: string) => {
    const d = mins(v) - currentMin;
  ...
  const todayCount = events.filter(
      (e) => e.status !== "cancelled" && e.status !== "moved",
    ).length,
    showNextStat = !!current && !!next;
  const breakStart = current ? mins(current.start) + 40 : 0,
    breakEnd = current ? mins(current.start) + 45 : 0,
    inBreak = !!current && currentMin >= breakStart && currentMin < breakEnd,
    breakFinished = !!current && currentMin >= breakEnd;
  const progress = current
    ...
    : 0;
  const focusCountdown = inBreak
    ? countdown(
        `${String(Math.floor(breakEnd / 60)).padStart(2, "0")}:${String(breakEnd % 60).padStart(2, "0")}`,
      )
    : current
  ...
  const saveNote = async () => {
    if (!person || !noteEvent) return;
    const nextNotes = { ...notes };
    if (noteText.trim()) nextNotes[noteEvent.key] = noteText.trim();
    else delete nextNotes[noteEvent.key];
    setSaving(true);
    try {
  ...
  return (
    <main data-no-pairs={noPairs ? "true" : undefined} className="dashboard-v2">
      <style jsx global>{`
        ...
        .dashboard-v2-head > div > p:last-child {
          margin: 9px 0 0;
          color: var(--muted);
          font-size: 14px;
        }
        ...
        .dashboard-tomorrow > div {
          display: grid;
          gap: 4px;
        }
        .dashboard-tomorrow span {
          color: var(--muted);
          font-size: 9px;
          font-weight: 800;
          letter-spacing: 0.1em;
        }
        .dashboard-tomorrow strong {
          font-size: 16px;
        }
        .dashboard-tomorrow small {
          color: var(--muted);
          font-size: 11px;
        }
        .dashboard-tomorrow a {
          color: var(--muted);
          font-size: 11px;
          text-decoration: none;
          white-space: nowrap;
        }
        .dashboard-tomorrow a:hover {
          color: var(--text);
        }
        .dashboard-stats {
          display: flex;
        ...
        .dashboard-event-v2.current {
          background: var(--surface-raised);
          border-color: var(--muted);
        }
        .dashboard-event-v2.past {
          opacity: 0.42;
          filter: grayscale(0.8);
        }
        .dashboard-event-v2.individual {
        ...
        .dashboard-seminar-grid {
          display: grid;
          grid-template-columns: repeat(2, minmax(0, 1fr));
          gap: 9px;
        }
        .dashboard-seminar-card {
          display: grid;
          gap: 5px;
          padding: 16px;
        ...
        .dashboard-seminar-card:hover {
          border-color: rgba(154, 230, 180, 0.7);
          transform: translateY(-1px);
        }
        .dashboard-seminar-card > span {
          color: #9ae6b4;
          font-size: 9px;
          font-weight: 800;
        ...
        .dashboard-seminar-card strong {
          font-size: 14px;
          line-height: 1.4;
        }
        .dashboard-seminar-card small {
          color: var(--muted);
          font-size: 10px;
          line-height: 1.5;
        }
        ...
        .dashboard-choose > p:not(.dashboard-kicker) {
          color: var(--muted);
        }
        .dashboard-people {
          display: grid;
        ...
        .dashboard-people button:hover {
          background: var(--surface-raised);
          transform: translateY(-1px);
        }
        .dashboard-people span {
          display: grid;
          place-items: center;
          width: 36px;
          height: 36px;
        ...
        .dashboard-note-modal > div {
          display: flex;
          justify-content: space-between;
          align-items: center;
        }
        .dashboard-note-modal > div > span {
          color: var(--muted);
          font-size: 9px;
          font-weight: 800;
          letter-spacing: 0.1em;
        }
        .dashboard-note-modal > div > button {
          border: 0;
          background: none;
          color: var(--muted);
          font-size: 24px;
        ...
        .dashboard-note-modal h2 {
          margin: 14px 0 3px;
        }
        .dashboard-note-modal > p {
          margin: 0;
          color: var(--muted);
          font-size: 11px;
        }
        .dashboard-note-modal textarea {
          width: 100%;
          min-height: 130px;
          margin-top: 15px;
          padding: 12px;
        ...
        .dashboard-note-modal footer {
          display: flex;
          justify-content: flex-end;
          gap: 7px;
          margin-top: 10px;
        }
        .dashboard-note-modal footer button {
          border: 1px solid var(--border);
          border-radius: 10px;
          padding: 9px 12px;
          color: var(--muted);
        ...
        .dashboard-note-modal footer button:last-child {
          color: var(--accent-text);
          background: var(--accent);
        }
        body:has(.dashboard-v2[data-no-pairs="true"]) .dashboard-rehearsals {
        ...
        @media (max-width: 650px) {
          ...
          .dashboard-note-modal > div:first-child {
            position: sticky;
            top: 0;
            z-index: 2;
            margin: calc(-20px - env(safe-area-inset-top)) -18px 0;
      ...
      <header className="dashboard-v2-head">
        <div>
          <p className="dashboard-kicker">214Р</p>
          <h1>{nightText ?? `Привет, ${person.name.split(" ")[0]}.`}</h1>
          <p>
      ...
      {(noPairs || current || next) && (
        <section className={`dashboard-focus${current ? " is-current" : ""}`}>
          <div className="dashboard-focus-top">
            ...
            {current && <em>{progress}%</em>}
          </div>
          <h2>
            {current?.title ??
              (gapText ? gapText : next?.title) ??
              "гуляем, товарищи"}
          </h2>
          {current ? (
            <>
              ...
              <strong>
                {inBreak
                  ? `до конца перерыва · ${focusCountdown}`
                  : `до конца · ${focusCountdown}`}
              </strong>
              <div
                className={`dashboard-progress${breakFinished ? " break-finished" : ""}`}
              >
            ...
            <>
              <p>
                {gapText
                  ? `Следующая · ${next.title}`
                  : `${next.subtitle} · ${next.detail}`}
              </p>
              <strong>до начала · {countdown(next.start)}</strong>
            </>
          ) : (
            <p>Пар сегодня нет.</p>
          )}
      ...
      {dashboardVisible("tomorrow") && !noPairs && showTomorrow && (
        <section className="dashboard-tomorrow" style={{order:dashboardOrder("tomorrow")}}>
          <div>
            ...
            <small>
              {tomorrowEvents.length
                ? `Первая в ${tomorrowEvents[0].start} · ${tomorrowEvents.length} ${tomorrowEvents.length === 1 ? "занятие" : "занятия"} · до ${tomorrowEvents[tomorrowEvents.length - 1].end}`
        ...
        >
          {showNextStat && (
            <article className="dashboard-stat-next">
              ...
              <small>
                {next
                  ? `через ${countdown(next.start)} · ${next.start}–${next.end}`
                  : "Больше пар нет"}
              </small>
            </article>
          )}
      ...
      {dashboardVisible("today") && !noPairs && (
        <section className="dashboard-timeline" style={{order:dashboardOrder("today")}}>
          ...
          {events.length ? (
            events.map((e) => (
              ...
              >
                ...
                <div className="dashboard-event-content">
                  ...
                  {(e.note || notes[e.key]) && (
                    <small className="dashboard-note-preview">
                      ● {e.note || notes[e.key]}
                    </small>
                  )}
                </div>
            ...
            <div className="dashboard-empty">На сегодня занятий нет.</div>
          )}
        </section>
      )}
      {dashboardVisible("seminars") && <section className="dashboard-seminars" style={{order:dashboardOrder("seminars")}}>
        ...
        {mySeminars.length ? (
          <div className="dashboard-seminar-grid">
            {mySeminars.map((item) => (
              <Link
                className="dashboard-seminar-card"
                href="/seminars"
                key={`${item.listId}-${item.topicId}`}
              >
                <span>
                  {item.subject} · {item.listTitle}
                </span>
                <strong>{item.topicTitle}</strong>
                <small>
                  {item.participants.length > 1
                    ? `Участники: ${item.participants.join(" · ")}`
                    : "Ты записан на эту тему"}
                </small>
              </Link>
            ))}
          </div>
          ...
          <div className="dashboard-empty">
            Ты пока не записан на семинары.{" "}
        ...
        createPortal(
          <AnimatePresence>
            {noteEvent && (
              ...
              >
                <motion.section
                  className="dashboard-note-modal"
                  role="dialog"
                  aria-modal="true"
                  aria-label="Личная заметка"
                  onMouseDown={(e) => e.stopPropagation()}
                  initial={
                    reducedMotion ? false : { opacity: 0, y: 14, scale: 0.985 }
                  }
                ...
                >
                  ...
                  <p>
                    {noteEvent.start}–{noteEvent.end} · {noteEvent.subtitle}
                  </p>
                  <textarea
                    value={noteText}
                    onChange={(e) => setNoteText(e.target.value)}
                    placeholder="Например: взять конспект…"
                  />
                  <footer>
                    <button onClick={() => setNoteEvent(null)}>Отмена</button>
