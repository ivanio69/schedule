    ...
    <>
      <article className="schedule-card" role={onClick ? "button" : undefined} tabIndex={onClick ? 0 : undefined} onClick={onClick} onKeyDown={(event) => { if (onClick && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); onClick(); } }}>
        ...
        <div className="schedule-card__body">
          <div className="schedule-card__title-row"><div className="schedule-card__title"><span className="card-kind-badge card-kind-badge--lesson">ПАРА</span><h2>{lesson.class}</h2></div><span className={`group-badge ${shared ? "group-badge--shared" : ""}`}>{shared ? "Обе группы" : `${lesson.group[0]} подгруппа`}</span></div>
