...
export default function NotificationCenter({ personId }: { personId: string }) {
  ...
  return <>
    ...
    <style jsx global>{`
      ...
      .notification-center-panel{position:relative;display:grid;grid-template-rows:auto minmax(0,1fr) auto;width:min(100%,620px);max-height:min(760px,calc(100dvh - 40px));min-height:0;overflow:hidden;border:1px solid #34343b;border-radius:24px;color:#f5f5f5;background:#0f0f12;box-shadow:0 30px 100px rgba(0,0,0,.72),0 0 0 1px rgba(255,255,255,.025)}
      .notification-center-panel>header{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;min-width:0;padding:22px 22px 18px;border-bottom:1px solid #242429;background:#0f0f12}
      ...
      .notification-center-panel>header>button{display:grid;place-items:center;flex:0 0 auto;width:36px;height:36px;margin:-4px -6px 0 0;border:0;border-radius:10px;background:transparent;color:#94949d;font-size:27px;line-height:1;cursor:pointer}
      ...
      .notification-center-panel>footer{display:flex;justify-content:flex-end;padding:14px 18px;border-top:1px solid #242429;background:#0f0f12}
