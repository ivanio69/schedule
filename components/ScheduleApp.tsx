...
export default function ScheduleApp() {
  ...
  const setEventReminder=async(value:5|10|15|30)=>{if(!creatorId||!details)return;const key=reminderKey(details),next={...eventReminders,[key]:value};setEventReminders(next);try{await fetch("/api/profile/settings",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({personId:creatorId,eventReminders:next})})}catch{}};
  ...
  const shareDay=async()=>{const lines=entries.map(entry=>`${entry.time} · ${entry.type==="lesson"?entry.item.class:entry.item.subject}`);const title=`214Р · ${formatDate(selectedDate)}`;const text=[title,...lines].join("\n");try{if(navigator.share)await navigator.share({title,text});else await navigator.clipboard.writeText(text)}catch{}};
