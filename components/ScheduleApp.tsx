...
export default function ScheduleApp() {
  ...
  useEffect(() => { const id=getCreatorId(); setCreatorId(id); if(id) void fetch(`/api/profile/settings?personId=${encodeURIComponent(id)}`,{cache:"no-store"}).then(r=>r.ok?r.json():null).then(data=>setChinaMode(data?.chinaMode===true)).catch(()=>{}); }, []);
  useEffect(() => {
    void (async () => {
      try {
