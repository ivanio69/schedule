...
export default function ScheduleApp() {
  ...
  const entries = [...lessons.map(item => ({ type: "lesson" as const, time: item.timeStart, item })), ...rehearsals.map(item => ({ type: "rehearsal" as const, time: item.timeStart, item }))].sort((a,b) => a.time.localeCompare(b.time));
  ...
  return <MotionConfig reducedMotion="user"><main className="schedule-shell">
    ...
    <AnimatePresence>{details&&<motion.div className="details-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)setDetails(null)}} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><motion.section className="details-modal" role="dialog" aria-modal="true" aria-labelledby="details-title" initial={{opacity:0,y:28,scale:.94}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:18,scale:.97}} transition={{type:"spring",stiffness:420,damping:28,mass:.65}}><div className="details-modal__header"><div><span className="details-modal__eyebrow">{details.type==="lesson"?"Занятие":"Репетиция"}</span><h2 id="details-title">{details.type==="lesson"?details.item.class:details.item.subject}</h2></div><button type="button" className="details-modal__close" onClick={()=>setDetails(null)} aria-label="Закрыть">×</button></div><div className="details-modal__time"><strong>{details.item.timeStart}</strong><span>—</span><span>{details.item.timeEnd}</span></div><dl className="details-modal__list"><div><dt>Дата</dt><dd>{formatDate(details.type==="lesson"?selectedDate:details.item.date)}</dd></div>{details.type==="lesson"?<><div><dt>Преподаватель</dt><dd>{details.item.professor}</dd></div><div><dt>Аудитория</dt><dd>{details.item.auditorium}</dd></div><div><dt>Подгруппа</dt><dd>{details.item.group.length===2?"Обе группы":`${details.item.group[0]} подгруппа`}</dd></div></>:<><div><dt>Автор</dt><dd>{details.item.creatorName??"Автор не указан"}</dd></div><div><dt>Ответственный</dt><dd>{details.item.responsible}</dd></div><div><dt>Участники</dt><dd className="details-modal__participants">{details.item.participants.length?details.item.participants.map(p=><span key={p}>{p}</span>):<span>Не указаны</span>}</dd></div></>}</dl>{details.type==="rehearsal"&&details.item.creatorId===creatorId&&<button type="button" className="details-modal__delete" onClick={()=>void removeRehearsal(details.item.id)}>Удалить репетицию</button>}</motion.section></motion.div>}</AnimatePresence>
  </main></MotionConfig>;
}
