...
export default function ScheduleApp() {
  ...
  const [eventReminders,setEventReminders]=useState<Record<string,5|10|15|30>>({});
  ...
  useEffect(() => { const id=getCreatorId(); setCreatorId(id); if(id) void fetch(`/api/profile/settings?personId=${encodeURIComponent(id)}`,{cache:"no-store"}).then(r=>r.ok?r.json():null).then(data=>{setChinaMode(data?.chinaMode===true);setEventReminders(data?.eventReminders??{})}).catch(()=>{}); }, []);
  useEffect(() => {
    void (async () => {
      try {
  ...
  const reminderKey=(value:Details)=>value.type==="lesson"?`lesson:${selectedDate}:${value.item.timeStart}:${value.item.class}`:`${value.type}:${value.item.id}`;
  const setEventReminder=async(value:5|10|15|30|null)=>{if(!creatorId||!details)return;const key=reminderKey(details),previous=eventReminders,next={...eventReminders};if(value===null)delete next[key];else next[key]=value;setEventReminders(next);try{const response=await fetch("/api/profile/settings",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({personId:creatorId,eventReminders:next})});if(!response.ok)throw new Error()}catch{setEventReminders(previous)}};
