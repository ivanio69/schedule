...
function readPreferences(subjects: { name: string }[]): Preferences {
  ...
  try {
    ...
    return Object.fromEntries(subjects.map(({ name }) => [
      name,
      saved[name] === "1" || saved[name] === "2" || saved[name] === "both" ? saved[name] as GroupPreference : "both" as GroupPreference,
    ])) as Preferences;
  } catch {
    return defaults;
  }
...
export default function ScheduleApp() {
  ...
  useEffect(() => {
    if (Object.keys(preferences).length > 0) window.localStorage.setItem(SETTINGS_STORAGE_KEY, JSON.stringify(preferences));
  }, [preferences]);
  ...
  return (
    <MotionConfig reducedMotion="user">
      <main className="schedule-shell">
        ...
        <nav className="bottom-nav" aria-label="Разделы"><button type="button" className={view === "schedule" ? "is-active" : ""} aria-current={view === "schedule" ? "page" : undefined} onClick={() => setView("schedule")}><span aria-hidden="true">▦</span>Расписание</button><button type="button" className={view === "settings" ? "is-active" : ""} aria-current={view === "settings" ? "page" : undefined} onClick={() => setView("settings")}><span aria-hidden="true">⚙</span>Настройки</button></nav>
      </main>
    </MotionConfig>
  );
}
