...
export default function ScheduleApp() {
  ...
  return <MotionConfig reducedMotion="user"><main className="schedule-shell"><header className="schedule-header"><div><p className="eyebrow">214Р · расписание</p><h1>{view === "schedule" ? "Учебная неделя" : "Настройки"}</h1>{view === "schedule" && <p className="week-caption">{formatWeekRange(schedule, week)}</p>}</div></header>{view === "schedule" ? <><nav className="day-tabs" aria-label="Дни недели">{DAY_NAMES.map((name, index) => <button type="button" key={name} className={day === index ? "is-active" : ""} aria-current={day === index ? "page" : undefined} onClick={() => setDay(index)}><span>{name}</span><small>{index + 1}</small></button>)}</nav><section className="schedule-panel" {...handlers} aria-live="polite"><div className="week-toolbar"><button type="button" className="icon-button" onClick={() => setWeek((value) => Math.max(1, value - 1))} disabled={week === 1} aria-label="Предыдущая неделя">←</button><button type="button" className="week-number" onClick={() => setWeek(currentWeek)} aria-label="Перейти к текущей неделе"><span>Неделя</span><strong>{week}</strong>{week === currentWeek && <em>сейчас</em>}</button><button type="button" className="icon-button" onClick={() => setWeek((value) => Math.min(totalWeeks, value + 1))} disabled={week === totalWeeks} aria-label="Следующая неделя">→</button></div>{rehearsalOpen ? <section className="rehearsal-form"><div><p className="rehearsal-label">Новая репетиция</p><h2>{selectedDate}</h2></div><div className="rehearsal-grid"><label>Предмет<input className="admin-input" value={rehearsalSubject} onChange={(e) => setRehearsalSubject(e.target.value)} placeholder="Название предмета" autoFocus /></label><label>Ответственный<input className="admin-input" value={rehearsalResponsible} onChange={(e) => setRehearsalResponsible(e.target.value)} placeholder="Ответственный" /></label><label>Начало<input className="admin-input" type="time" value={rehearsalStart} onChange={(e) => setRehearsalStart(e.target.value)} /></label><label>Конец<input className="admin-input" type="time" value={rehearsalEnd} onChange={(e) => setRehearsalEnd(e.target.value)} /></label><label className="rehearsal-participants">Участники<input className="admin-input" value={rehearsalParticipants} onChange={(e) => setRehearsalParticipants(e.target.value)} placeholder="Имена участников" /><small>Укажите через запятую</small></label></div>{rehearsalError && <p className="admin-error">{rehearsalError}</p>}<div className="rehearsal-form__actions"><button type="button" className="admin-secondary" onClick={() => setRehearsalOpen(false)}>Отмена</button><button type="button" className="admin-primary" disabled={rehearsalSaving} onClick={() => void createRehearsal()}>{rehearsalSaving ? "Создаю…" : "Создать репетицию"}</button></div></section> : <button type="button" className="add-rehearsal-button" onClick={() => { setRehearsalError(""); setRehearsalOpen(true); }}>＋ Добавить репетицию</button>}<AnimatePresence mode="wait" initial={false}><motion.div key={`${week}-${day}-${JSON.stringify(preferences)}`} className="lesson-list" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: .18 }}>{entries.map((entry) => entry.type === "lesson" ? <ScheduleCard key={`lesson-${entry.item.timeStart}-${entry.item.class}-${entry.item.auditorium}-${entry.item.group.join(",")}`} lesson={entry.item} onClick={() => setDetails({ type: "lesson", item: entry.item })} /> : <RehearsalCard key={entry.item.id} rehearsal={entry.item} own={entry.item.creatorId === creatorId} onDelete={entry.item.creatorId === creatorId ? () => void removeRehearsal(entry.item.id) : undefined} onClick={() => setDetails({ type: "rehearsal", item: entry.item })} />)}{entries.length === 0 && <div className="empty-state"><span className="empty-state__icon">—</span><h2>Ничего нет</h2><p>В этот день ничего не запланировано.</p></div>}</motion.div></AnimatePresence></section><footer className="schedule-footer"><span>{DAY_NAMES[day]}</span><span>Нажмите на занятие для подробностей</span></footer></> : <section className="settings-panel" aria-label="Настройки расписания"><div className="settings-section"><div><p className="settings-section__eyebrow">Подгруппы</p><h2>Настройки предметов</h2><p>Для каждого предмета с подгруппами выберите, какую группу показывать.</p></div><div className="settings-list">{subgroupSubjects.map(({ name, groups }) => { const value = preferences[name] ?? "both"; return <div className="setting-row setting-row--subject" key={name}><span><strong>{name}</strong><small>Подгруппы: {groups.join(" и ")}</small></span><div className="preference-switch" role="group" aria-label={`Подгруппа для предмета ${name}`}>{(["1", "2", "both"] as GroupPreference[]).map((option) => <button type="button" key={option} className={value === option ? "is-active" : ""} aria-pressed={value === option} onClick={() => setPreference(name, option)}>{option === "both" ? "Обе" : option}</button>)}</div></div>; })}</div></div></section>}<nav className="bottom-nav" aria-label="Разделы"><button type="button" className={view === "schedule" ? "is-active" : ""} aria-current={view === "schedule" ? "page" : undefined} onClick={() => setView("schedule")}>Расписание</button><button type="button" className={view === "settings" ? "is-active" : ""} aria-current={view === "settings" ? "page" : undefined} onClick={() => setView("settings")}>Настройки</button></nav><AnimatePresence>{details && <motion.div className="details-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setDetails(null); }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .2, ease: "easeOut" }}><motion.section className="details-modal" role="dialog" aria-modal="true" aria-labelledby="details-title" initial={{ opacity: 0, y: 28, scale: .94 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 18, scale: .97 }} transition={{ type: "spring", stiffness: 420, damping: 28, mass: .65 }}><style>{`.details-backdrop{position:fixed;inset:0;z-index:100;display:flex;align-items:flex-end;justify-content:center;padding:16px;background:rgba(0,0,0,.72);backdrop-filter:blur(8px)}.details-modal{position:relative;width:min(100%,520px);max-height:min(82vh,680px);overflow:auto;padding:28px 24px 24px;border:1px solid var(--border);border-radius:24px;background:linear-gradient(145deg,#17171b,#0f0f12);box-shadow:0 24px 80px rgba(0,0,0,.5)}.details-modal__header{display:flex;align-items:flex-start;justify-content:space-between;gap:16px}.details-modal__eyebrow{display:block;margin:0 0 8px;color:var(--muted);font-size:10px;font-weight:800;letter-spacing:.1em;text-transform:uppercase}.details-modal h2{margin:0;padding-right:8px;font-size:25px;line-height:1.2;letter-spacing:-.035em}.details-modal__close{flex:0 0 auto;width:34px;height:34px;border:1px solid var(--border);border-radius:10px;color:var(--muted);background:var(--surface);cursor:pointer;font-size:22px;line-height:1}.details-modal__close:hover{color:var(--text);background:var(--surface-hover)}.details-modal__time{display:flex;align-items:center;gap:10px;margin:24px 0;padding:16px;border:1px solid var(--border);border-radius:16px;background:rgba(255,255,255,.035)}.details-modal__time strong{font-size:21px;letter-spacing:-.03em}.details-modal__time span{color:var(--muted)}.details-modal__list{display:grid;margin:0;border-top:1px solid var(--border)}.details-modal__list>div{display:grid;grid-template-columns:130px 1fr;gap:16px;padding:13px 0;border-bottom:1px solid var(--border)}.details-modal__list dt{color:var(--muted);font-size:11px}.details-modal__list dd{margin:0;color:var(--text);font-size:13px;line-height:1.45;text-align:right}.details-modal__participants{display:flex!important;flex-wrap:wrap;justify-content:flex-end;gap:5px}.details-modal__participants span{padding:4px 8px;border:1px solid var(--border);border-radius:999px;background:rgba(255,255,255,.035)}.details-modal__delete{width:100%;margin-top:18px;padding:10px 13px;border:1px solid rgba(255,155,155,.25);border-radius:11px;color:#ff9b9b;background:transparent;cursor:pointer;font-size:12px;font-weight:700}.details-modal__delete:hover{background:rgba(255,155,155,.08)}@media (min-width:601px){.details-backdrop{align-items:center}}@media (max-width:600px){.details-modal{padding:25px 18px 18px;border-radius:22px}.details-modal__list>div{grid-template-columns:105px 1fr}.details-modal__time{margin:20px 0}}`}</style><div className="details-modal__header"><div><span className="details-modal__eyebrow">{details.type === "lesson" ? "Занятие" : "Репетиция"}</span><h2 id="details-title">{details.type === "lesson" ? details.item.class : details.item.subject}</h2></div><button type="button" className="details-modal__close" onClick={() => setDetails(null)} aria-label="Закрыть">×</button></div><div className="details-modal__time"><strong>{details.item.timeStart}</strong><span>—</span><span>{details.item.timeEnd}</span></div><dl className="details-modal__list"><div><dt>Дата</dt><dd>{formatDate(details.type === "lesson" ? selectedDate : details.item.date)}</dd></div>{details.type === "lesson" ? <><div><dt>Преподаватель</dt><dd>{details.item.professor}</dd></div><div><dt>Аудитория</dt><dd>{details.item.auditorium}</dd></div><div><dt>Подгруппа</dt><dd>{details.item.group.length === 2 ? "Обе группы" : `${details.item.group[0]} подгруппа`}</dd></div></> : <><div><dt>Ответственный</dt><dd>{details.item.responsible}</dd></div><div><dt>Участники</dt><dd className="details-modal__participants">{details.item.participants.length ? details.item.participants.map((participant) => <span key={participant}>{participant}</span>) : <span>Не указаны</span>}</dd></div></>}</dl>{details.type === "rehearsal" && details.item.creatorId === creatorId && <button type="button" className="details-modal__delete" onClick={() => void removeRehearsal(details.item.id)}>Удалить репетицию</button>}</motion.section></motion.div>}</AnimatePresence></main></MotionConfig>;
}
