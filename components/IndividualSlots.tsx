...
const mine=useMemo(()=>slots.filter(s=>s.studentIds?.includes(personId??"")||s.studentId===personId),[slots,personId]);const available=useMemo(()=>slots.filter(s=>(s.studentIds?.length??(s.studentId?1:0))<(s.capacity??1)),[slots]);const groups=useMemo(()=>{const map=new Map<string,Slot[]>();for(const s of slots){const a=map.get(s.subject)??[];a.push(s);map.set(s.subject,a)}return [...map.entries()]},[slots]);const loadingNode=<LoadingState screen label="Загружаем индивидуальные" detail="Получаем доступные занятия и твои записи."/>;if(loading)return <AsyncContentTransition loading loadingNode={loadingNode}><div /></AsyncContentTransition>;if(!personId)return <AsyncContentTransition loading={false} loadingNode={loadingNode}><section className="student-slots"><div className="student-slots-empty"><h2>Твои индивидуальные занятия</h2><p><Link href="/">Выбери своё имя</Link>, чтобы увидеть занятия и записаться.</p></div></section></AsyncContentTransition>;const changeBooking=async(s:Slot,action:"claim"|"release")=>{
