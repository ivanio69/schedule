...
export function RehearsalCard({ rehearsal, own, onDelete, onClick }: { rehearsal: Rehearsal; own: boolean; onDelete?: () => void; onClick?: () => void }) {
  return (
    <article className={`rehearsal-card${rehearsal.isGlobal ? " rehearsal-card--global" : ""}`} role={onClick ? "button" : undefined} tabIndex={onClick ? 0 : undefined} onClick={onClick} onKeyDown={(event) => { if (onClick && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); onClick(); } }}>
      ...
      <div className="schedule-card__body">
        <div className="schedule-card__title-row">
          <div className="schedule-card__title"><span className={`card-kind-badge card-kind-badge--rehearsal${rehearsal.isGlobal ? " card-kind-badge--global" : ""}`}>{rehearsal.isGlobal ? "ОБЩАЯ РЕПА" : "РЕПА"}</span><h2>{rehearsal.subject}</h2></div>
