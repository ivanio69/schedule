...
export default function Seminars({ admin = false }: { admin?: boolean }) {
  ...
  return <section className="seminars">
    <header className="seminars-heading"><div><p>СЕМИНАРЫ</p><h1>{admin ? "Темы семинаров" : "Забить тему"}</h1><span>{admin ? "Создавай списки и управляй участниками." : "Выбирай темы по предметам. Все видят, кто записан."}</span></div><button disabled={busy} onClick={() => void load()}>Обновить</button></header>
    {error && <p role="alert" className="seminar-message">{error}</p>}
    {notice && <p role="status" className="seminar-message">{notice}</p>}
    {admin && <form className="seminar-create" onSubmit={async e => {
      e.preventDefault();
      ...
      <label>Участников на одну тему<select value={capacity} onChange={e => setCapacity(Number(e.target.value))}><option value={1}>Один человек</option><option value={2}>Два человека</option></select></label></div>
    ...
    {groups.map(group => <section key={group} className="seminar-subject"><h2>{group}</h2>{lists.filter(l => l.subject === group).map(list => <article key={list.id} className="seminar-list">
      <header><h3>{list.title}</h3><span>{list.capacity === 1 ? "По одному человеку" : "До двух человек на тему"}</span></header>
      <ol>{list.topics.map((topic, index) => {
        ...
        return <li key={topic.id} className={mine ? "seminar-mine" : ""}>
          ...
          {admin ? <button disabled={busy} onClick={() => edit(list, topic)}>Участники</button> : mine ? <button disabled={busy} onClick={() => void mutate("POST", { action: "release", listId: list.id, topicId: topic.id, studentId: personId }, "Запись отменена")}>Отменить запись</button> : <button className={!full ? "seminar-primary" : ""} disabled={busy || full || !personId} onClick={() => void mutate("POST", { action: "claim", listId: list.id, topicId: topic.id, studentId: personId }, "Тема за тобой")}>{full ? "Занято" : "Забить тему"}</button>}</div>
          {editing && <div className="seminar-editor"><p>Выбери до {list.capacity} участников. Можно освободить все места.</p>
