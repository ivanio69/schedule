...
export default function PushPrompt(){
 ...
 if(!show)return null;return <div className="push-prompt-backdrop"><section className="push-prompt"><h2>Не пропускай изменения</h2><p>Включи уведомления о семинарах, индивидуальных занятиях и расписании.</p>{error&&<p role="alert">{error}</p>}<button onClick={()=>void enable()}>Включить уведомления</button><button onClick={()=>{localStorage.setItem(DISMISSED_KEY,String(Date.now()));setShow(false)}}>Не сейчас</button></section></div>;
}
