...
export default function HeadmanAnnouncements({ people }: { people:Person[] }){
  ...
  const [loading,setLoading]=useState(true),[busy,setBusy]=useState(false),[editingId,setEditingId]=useState<string|null>(null),[status,setStatus]=useState("");
  const [title,setTitle]=useState(""),[body,setBody]=useState(""),[audience,setAudience]=useState<"all"|"selected">("all"),[selected,setSelected]=useState<string[]>([]),[query,setQuery]=useState(""),[endsAt,setEndsAt]=useState(""),[sendPush,setSendPush]=useState(false);
  ...
  const reset=()=>{setEditingId(null);setTitle("");setBody("");setAudience("all");setSelected([]);setQuery("");setEndsAt("");setAccentColor(DEFAULT_ANNOUNCEMENT_ACCENT);setBackgroundColor(DEFAULT_ANNOUNCEMENT_BACKGROUND);setSendPush(false)};
  const edit=(item:DashboardAnnouncement)=>{const colors=announcementColors(item);setEditingId(item.id);setTitle(item.title);setBody(item.body);setAudience(item.audience);setSelected(item.recipientIds);setEndsAt(localDateTimeValue(item.endsAt));setAccentColor(colors.accentColor);setBackgroundColor(colors.backgroundColor);setSendPush(false);setStatus("");document.getElementById("headman-announcement-editor")?.scrollIntoView({behavior:"smooth",block:"start"})};

  const save=async()=>{if(busy||!title.trim()||!body.trim()||(audience==="selected"&&!selected.length))return;setBusy(true);setStatus("");try{const response=await fetch("/api/headman/announcements",{method:editingId?"PATCH":"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...(editingId?{id:editingId}:{}),title,body,audience,recipientIds:selected,endsAt:endsAt?new Date(endsAt).toISOString():null,accentColor,backgroundColor,sendPush})});const data=await response.json().catch(()=>null);if(!response.ok)throw new Error(data?.error??"Не удалось сохранить объявление");setStatus((editingId?"Объявление обновлено":"Объявление опубликовано")+(sendPush?data.push?.error?" · Push не отправлен":" · Push "+(data.push?.sent??0)+"/"+(data.push?.subscriptions??0):""));reset();await load()}catch(error){setStatus(error instanceof Error?error.message:"Не удалось сохранить объявление")}finally{setBusy(false)}};
  ...
  return <section className={styles.root}>
    <header className={styles.sectionHead}><div><p className="eyebrow">Объявления</p><h2>Полное управление</h2><span>Создание, редактирование, аудитория, оформление, Push и архив.</span></div><button type="button" onClick={()=>void load()} disabled={loading||busy}>↻ Обновить</button></header>
    <div className={styles.layout}>
      <section className={styles.editor} id="headman-announcement-editor">
        <div className={styles.editorHead}><div><span>{editingId?"РЕДАКТИРОВАНИЕ":"НОВОЕ ОБЪЯВЛЕНИЕ"}</span><h3>{editingId?"Изменить объявление":"Сообщение на дашборде"}</h3></div>{editingId&&<button type="button" onClick={reset}>Отмена</button>}</div>
        ...
        {audience==="selected"&&<div className={styles.peoplePicker}><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Найти человека…"/><div>{filtered.map(person=><button type="button" key={person.id} className={selected.includes(person.id)?styles.active:""} onClick={()=>togglePerson(person.id)}>{person.name}</button>)}</div></div>}
        ...
        <div className={styles.preview} style={previewStyle}><span>ПРЕДПРОСМОТР</span><strong>{title.trim()||"Заголовок объявления"}</strong><p>{body.trim()||"Здесь будет текст объявления."}</p></div>
        ...
        <button type="button" className={styles.pushToggle} aria-pressed={sendPush} onClick={()=>setSendPush(value=>!value)}><span><strong>{editingId?"Отправить Push после сохранения":"Отправить Push вместе с публикацией"}</strong><small>Тем же пользователям, которым показывается объявление</small></span><i><u/></i></button>
