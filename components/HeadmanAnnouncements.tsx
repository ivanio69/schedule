...
export default function HeadmanAnnouncements({ people }: { people:Person[] }){
  ...
  const reset=()=>{setEditingId(null);setEditorOpen(false);setTitle("");setBody("");setAudience("all");setSelected([]);setQuery("");setEndsAt("");setAccentColor(DEFAULT_ANNOUNCEMENT_ACCENT);setBackgroundColor(DEFAULT_ANNOUNCEMENT_BACKGROUND);setSendPush(false)};
  ...
  const edit=(item:DashboardAnnouncement)=>{const colors=announcementColors(item);setEditingId(item.id);setEditorOpen(true);setTitle(item.title);setBody(item.body);setAudience(item.audience);setSelected(item.recipientIds);setEndsAt(localDateTimeValue(item.endsAt));setAccentColor(colors.accentColor);setBackgroundColor(colors.backgroundColor);setSendPush(false);setStatus("");requestAnimationFrame(()=>document.getElementById("headman-announcement-editor")?.scrollIntoView({behavior:"smooth",block:"nearest"}))};
