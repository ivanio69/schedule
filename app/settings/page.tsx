...
export default function SettingsPage() {
  ...
  const [widgetDevices,setWidgetDevices]=useState(0);
  ...
  useEffect(() => {
    if (!personId) { setWidgetDevices(0); return; }
    let stopped=false;
    void fetch("/api/widget/pair",{cache:"no-store"})
      .then(response=>response.ok?response.json():null)
      .then(data=>{if(!stopped)setWidgetDevices(Number(data?.connectedDevices??0));})
      .catch(()=>{});
    return()=>{stopped=true;};
  }, [personId]);
  ...
  const connectIOSWidget=async()=>{
    ...
    try{
      ...
      setWidgetNotice("Открываем companion-приложение…");
      window.location.href=data.deepLink;
    }catch(error){setWidgetNotice(error instanceof Error?error.message:"Не удалось подключить iPhone");}
    finally{setWidgetBusy(false);}
  };
  const revokeIOSWidgets=async()=>{
    if(widgetBusy)return;
    setWidgetBusy(true);setWidgetNotice("");
    try{
      const response=await fetch("/api/widget/pair",{method:"DELETE",cache:"no-store"});
      if(!response.ok)throw new Error();
      setWidgetDevices(0);setWidgetNotice("Все iOS-виджеты отключены");
    }catch{setWidgetNotice("Не удалось отключить виджеты");}
    finally{setWidgetBusy(false);}
  };
