...
export default function SettingsPage() {
  ...
  const [firstLessonReminder,setFirstLessonReminder]=useState<5|10|15|30>(15);
  const [firstLessonReminderEnabled,setFirstLessonReminderEnabled]=useState(true);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      if ([5,10,15,30].includes(Number(cloud?.firstLessonReminder))) setFirstLessonReminder(Number(cloud.firstLessonReminder) as 5|10|15|30);
      setFirstLessonReminderEnabled(cloud?.firstLessonReminderEnabled !== false);
  ...
  const setFirstLessonReminderEnabledPreference=async(value:boolean)=>{if(!personId)return;const previous=firstLessonReminderEnabled;setFirstLessonReminderEnabled(value);setStatus("Сохраняю…");try{const response=await fetch("/api/profile/settings",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({personId,firstLessonReminderEnabled:value})});if(!response.ok)throw new Error();setStatus("Сохранено в облаке")}catch{setFirstLessonReminderEnabled(previous);setStatus("Не удалось сохранить в облако")}};

  const setFirstLessonReminderPreference=async(value:5|10|15|30)=>{if(!personId)return;const previous=firstLessonReminder;setFirstLessonReminder(value);setStatus("Сохраняю…");try{const response=await fetch("/api/profile/settings",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({personId,firstLessonReminder:value})});if(!response.ok)throw new Error();setStatus("Сохранено в облаке")}catch{setFirstLessonReminder(previous);setStatus("Не удалось сохранить в облако")}};
