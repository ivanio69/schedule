...
import { DASHBOARD_SECTION_LABELS, DEFAULT_DASHBOARD_SETTINGS, normalizeDashboardSettings, type DashboardSectionId, type DashboardSettings } from "@/lib/dashboard-settings";
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      setDashboardSettings(normalizeDashboardSettings(cloud?.dashboardSettings));
  ...
  const saveDigestSettings=async(next:DigestSettings)=>{
    if(!personId)return;
    const previous=digestSettings;
    ...
    setDigestSettings(normalized);setStatus("Сохраняю…");
    try{
      ...
      if(!response.ok)throw new Error();
      setStatus("Дайджесты сохранены");
    }catch{
      setDigestSettings(previous);setStatus("Не удалось сохранить дайджесты");
    }
  };

  const saveDashboardSettings=async(next:DashboardSettings)=>{
    if(!personId)return;
    const previous=dashboardSettings;
    const normalized=normalizeDashboardSettings(next);
    setDashboardSettings(normalized);setStatus("Сохраняю главную…");
    try{
      const response=await fetch("/api/profile/settings",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({personId,dashboardSettings:normalized})});
      if(!response.ok)throw new Error();
      setStatus("Главная сохранена");
    }catch{setDashboardSettings(previous);setStatus("Не удалось сохранить главную")}
  };
  const toggleDashboardSection=(id:DashboardSectionId)=>{
    const hidden=dashboardSettings.hidden.includes(id)?dashboardSettings.hidden.filter(item=>item!==id):[...dashboardSettings.hidden,id];
    void saveDashboardSettings({...dashboardSettings,hidden});
  };
  const moveDashboardSection=(id:DashboardSectionId,direction:-1|1)=>{
    const index=dashboardSettings.order.indexOf(id);
    const target=index+direction;
    if(index<0||target<0||target>=dashboardSettings.order.length)return;
    const order=[...dashboardSettings.order];
    [order[index],order[target]]=[order[target],order[index]];
    void saveDashboardSettings({...dashboardSettings,order});
  };
