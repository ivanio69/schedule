...
export default function SettingsPage() {
  ...
  const saveDigestSettings=(next:DigestSettings)=>{
    ...
    void task.then(()=>{
      if(revision===digestSaveRevision.current)setStatus("Дайджесты сохранены");
    }).catch(()=>{
      if(revision===digestSaveRevision.current)setStatus("Не удалось сохранить дайджесты");
    });
  };
  ...
  const forceDigestPush=async()=>{
    ...
    setStatus("Отправляю тестовые дайджесты…");
    try{
      ...
      setStatus(error instanceof Error?error.message:"Не удалось отправить тестовый дайджест");
    }finally{
