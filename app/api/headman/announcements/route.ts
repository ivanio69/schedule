...
async function authorized(request: NextRequest) {
  return getRoleSessionPerson(request, ["headman", "admin"]);
}
...
async function audienceFrom(body: Record<string, unknown>, fallback?: DashboardAnnouncement) {
  ...
  if (fallback && !audienceTouched) {
    const recipientsForPush = fallback.audience === "all" ? people.map(person => person.id) : fallback.recipientIds.filter(id => validIds.has(id));
  ...
  const recipientIds = audience === "selected" ? [...new Set(source.filter((id: unknown): id is string => typeof id === "string" && validIds.has(id)))] : [];
...
async function maybeSendPush(requested: boolean, recipients: string[], title: string, body: string) {
  if (!requested) return null;
  try {
    return await sendPush(recipients, null, { title: title.slice(0, 80), body: body.slice(0, 240), url: "/" });
  } catch (error) {
    console.error("Failed to send headman announcement push", error);
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (endsAt && Date.parse(endsAt) <= Date.parse(current.startsAt)) return NextResponse.json({ error: "Окончание должно быть позже начала" }, { status: 400 });
