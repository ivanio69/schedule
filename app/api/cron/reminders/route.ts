...
export async function GET(request:NextRequest){
  ...
  for(const person of people){
    ...
    const firstEnabled=settings.firstLessonReminderEnabled!==false;
    ...
    if(firstEnabled&&first&&due(first.timeStart,lead,time))await once(`first:${date}:${person.id}:${lead}`,async()=>{const r=await sendPush([person.id],null,{title:"Скоро первая пара",body:`${first.class} · ${first.timeStart} · ${first.auditorium}`,url:"/"});sent+=r.sent});
