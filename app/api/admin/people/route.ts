...
export async function POST(request: NextRequest) {
  ...
  const person = await savePerson({ name: body.name.trim().slice(0,120), active: body.active !== false, role: normalizePersonRole(body.role) });
...
export async function PUT(request: NextRequest) {
  ...
  const person = await savePerson({ name: body.name.trim().slice(0,120), active: body.active !== false, role: normalizePersonRole(body.role) }, body.id);
