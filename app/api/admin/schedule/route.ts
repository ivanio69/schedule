...
type Schedule = {
  ...
  days: Array<{ name: string; table: Array<Record<string, unknown>> }>;
};
...
function validateSchedule(value: unknown): value is Schedule {
  ...
  if (!Array.isArray(schedule.semesterStart) || schedule.semesterStart.length !== 3) return false;
  if (!schedule.semesterStart.every((n) => Number.isInteger(n))) return false;
  if (!Array.isArray(schedule.days) || schedule.days.length === 0) return false;
  return schedule.days.every((day) =>
    Boolean(day && typeof day.name === "string" && Array.isArray(day.table)) &&
    day.table.every((lesson) => lesson && typeof lesson === "object"),
  );
}
...
export async function POST(request: NextRequest) {
  if (request.nextUrl.pathname.endsWith("/login")) return login(request);
  return unauthorized();
}
...
export async function PUT(request: NextRequest) {
  ...
  if (!body?.sha || !validateSchedule(body.schedule)) {
    return NextResponse.json({ error: "Invalid schedule or SHA" }, { status: 400 });
  }

  const content = `${JSON.stringify(body.schedule, null, 2)}\n`;
  const response = await githubRequest(`contents/${FILE_PATH}`, {
    method: "PUT",
    body: JSON.stringify({
