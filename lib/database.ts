...
export async function saveSchedule(schedule: ScheduleData) {
  const db = await getDatabase();
  await db.collection<ScheduleData & { _id: string }>("schedule").replaceOne(
    { _id: "current" },
    { ...schedule, _id: "current" },
    { upsert: true },
  );
}

export async function getRehearsals(week: number, day: number) {
  const db = await getDatabase();
  return db.collection<Rehearsal>("rehearsals")
    .find({ week, day })
    .sort({ timeStart: 1, createdAt: 1 })
    .toArray();
}
