...
import { normalizeDigestSettings, type DigestSettings } from "@/lib/digest-settings";
...
async function eventsFor(person:Person,date:string,schedule:ScheduleData,settings:ProfileSettings):Promise<DigestEvent[]>{
  ...
  const ownIndividuals=individuals.map<DigestEvent>(lesson=>({start:lesson.timeStart,title:lesson.title||"Индивидуальное",kind:"individual"}));
